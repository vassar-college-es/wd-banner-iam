create or replace PROCEDURE       WD_EDIT_pprccmt (
...
-- p47 : /worker_worker_id
   ...
   return_object varchar2(10) := 'PPRCCMT';
   return_status varchar2(100) := 'success';
      
   pprccmt_cmty pprccmt.pprccmt_cmty_code%TYPE := '';
...
BEGIN
    ...
    if p34 != 'No Update' then
        if p34 like '%Facu%' then
          pprccmt_cmty := 'MFA';
        end if;
        if p34 like '%Admin%' then
          pprccmt_cmty := 'MAD';
        end if;
        if p34 like '%Stude%' then
          pprccmt_cmty := 'MST';
        end if;
        if p34 like '%Staff%' then
          pprccmt_cmty := 'MST';
        end if;
        
    end if;

    if pprccmt_cmty IS NOT NULL then -- something to update; proceed
      
      begin
      delete from pprccmt where pprccmt_pidm = p0 and pprccmt_cmty_code = pprccmt_cmty;
      end;
      
      begin
      insert into pprccmt (pprccmt_pidm, pprccmt_cmty_code, pprccmt_text, pprccmt_activity_date)
      values (p0, pprccmt_cmty, '', sysdate);
