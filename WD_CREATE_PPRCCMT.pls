create or replace PROCEDURE       WD_CREATE_PPRCCMT (
...
BEGIN
   ...
   if p23 like '%Facu%' then
       pprccmt_cmty := 'MFA';
   end if;
   if p23 like '%Admin%' then
       pprccmt_cmty := 'MAD';
   end if;
   if p23 like '%Stude%' then
       pprccmt_cmty := 'MST';
   end if;
    if p23 like '%Staff%' then
       pprccmt_cmty := 'MST';
   end if; 
   
   if pprccmt_cmty is not null THEN
   begin
   insert into pprccmt (pprccmt_pidm, pprccmt_cmty_code, pprccmt_text, pprccmt_activity_date)
   values (p7, pprccmt_cmty, '', sysdate);
