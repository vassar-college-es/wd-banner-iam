create or replace PROCEDURE       WD_EDIT_SPRIDEN (
...
-- p47 : /worker_worker_id
   ...
   return_object varchar2(10) := 'SPRIDEN';
   return_status varchar2(100) := 'success';
   
   spriden_update varchar2(500) := '';
   spriden_set varchar2(250) := '';
   spriden_where varchar2(250) := '';
...
BEGIN
    ...
    if p0 = NULL then
      ...
      return_status := 'No Banner PIDM provided';
        ...
        if p9 != 'No Update' then
            spriden_set := spriden_set || 'spriden_first_name = ''' || p9 || ''', ';
        end if;
        if p10 != 'No Update' then
            spriden_set := spriden_set || 'spriden_last_name = ''' || p10 || ''', ';
        end if;
        if p11 != 'No Update' then
            spriden_set := spriden_set || 'spriden_mi = ''' || p11 || ''', ';
        end if;
        
    end if;
    
 
    if spriden_set IS NOT NULL then -- something to update; proceed
      spriden_set := spriden_set || 'spriden_activity_date = sysdate';
      spriden_where := 'spriden_change_ind is null and spriden_pidm = ' || p0;
   
      spriden_update := 'UPDATE spriden SET ' || spriden_set || ' WHERE ' || spriden_where;
      begin
      EXECUTE IMMEDIATE spriden_update;
      exception
         when OTHERS then
            return_code := '401';
      ...
      OPEN REF_CUR_OUT FOR
            SELECT DISTINCT p0 employee_pidm, return_code return_code, return_status return_status from dual;
   else
      OPEN REF_CUR_OUT FOR
